...
int main()
{
    ...
    //打开图片
    cv::Mat img = cv::imread("../img/pnp.jpg");
    if(img.empty())
    {
    ...
    for(size_t i=0; i<contours.size(); i++)
    {
        ...
        if(aspectRatio >= 1.3 && std::max(height, width)>36)
        {
            ...
            for(int j=0; j<4; j++)
            {
                cv::line(img, vertices[j], vertices[(j+1)%4], cv::Scalar(0,255,0), 5);
            }
        }
    }
    ...
    if(lightBars.size() == 2)
    {
        ...
        for(int i=0; i<4; i++)
        {
            cv::line(img, rectVertices[i], rectVertices[(i+1)%4], cv::Scalar(0,0,255), 2);
        }
